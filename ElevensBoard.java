...
public class ElevensBoard extends Board {
  ...
  private boolean containsPairSum11(List<Integer> selectedCards) {
    for (int i = 0; i < selectedCards.size(); i++) {
      for (int j = 0; j < selectedCards.size(); j++) {
        int first = selectedCards.get(i);
        Card first_card = cardAt(first);
        int second = selectedCards.get(j);
        Card second_card = cardAt(second);
        if (first_card.getPointValue() + second_card.getPointValue() == 11) {
          return true;
        } 
        else {
          continue;
  ...
  private boolean containsJQK(List<Integer> selectedCards) {
    ...
    for (int i = 0; i < selectedCards.size(); i++) {
      ...
      Card card_val = cardAt(index);
      if (card_val.getRank().equals("jack")) {
        a = a + 1;
      } else if (card_val.getRank().equals("queen")) {
        b = b + 1;
      } else if (card_val.getRank().equals("king")) {
        c = c + 1;
      }
    }
    if (a == 1 && b == 1 && c == 1) {
